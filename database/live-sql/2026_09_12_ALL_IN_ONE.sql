-- Everything live is missing, in one file.
--
-- Two things a git pull cannot carry: a column that arrived as a migration
-- (migrations need a console, and live has none), and the AI prompt rows,
-- which live in the database rather than in the code.
--
-- Touches nothing else. Safe to run more than once — every part checks first.
-- phpMyAdmin: pick the database, Import tab, choose this file, Go.
